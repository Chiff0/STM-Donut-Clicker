...
volatile TS_State_t latest_touch_state = {0};
volatile uint32_t touch_timestamp = 0;
...
void BSP_TS_Callback (uint32_t Instance)
{
    if (Instance == 0)
    {
        if (BSP_TS_GetState (0, (TS_State_t*) &latest_touch_state) == BSP_ERROR_NONE)
        {
            touch_event_pending = true;
            touch_timestamp = HAL_GetTick ();
        }
    }
}

bool process_touch_event (TS_State_t* processed_state)
{
    if (!touch_event_pending) return false;

    __disable_irq();
    *processed_state = latest_touch_state;
...
void DonutRotatus ()
{
    ...
    if (BSP_TS_EnableIT (0) != BSP_ERROR_NONE) Error_Handler();

    TS_State_t current_touch_state = {0};
    ...
    for (;;)
    {
		...
		bool touch_event_occurred = process_touch_event (&current_touch_state);

		if (touch_event_occurred)
		{
			if (current_touch_state.TouchDetected && !prev_touch_detected)
			{
				if (inactivity_counter > 0) inactivity_counter = 0;

				int x_corrected = current_touch_state.TouchY, y_corrected = current_touch_state.TouchX;
			...
			prev_touch_detected = current_touch_state.TouchDetected;
		}
		else
		{
			nums[counter++] = 0;
